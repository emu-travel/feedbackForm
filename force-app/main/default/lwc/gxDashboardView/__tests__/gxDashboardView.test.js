...
describe("export", () => {
  it("says how many responses went into which file", () => {
    expect(
      exportMessage({ rows: 12, total: 12, fileName: "golf-extra.csv" })
    ).toBe("12 responses saved to golf-extra.csv.");
  });

  it("says when the file holds only part of the selection", () => {
    expect(
      exportMessage({
        rows: 1000,
        total: 1400,
    ...
    ).toContain("the first 1000 of 1400");
  });

  it("says so when there is nothing to export", () => {
    expect(exportMessage({ rows: 0 })).toBe(
      "There are no responses in this selection to export."
    );
  });
});
